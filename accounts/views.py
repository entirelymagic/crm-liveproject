...
def register_page(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'Dear '+username + ', you have successful created an account!')
...
def home(request):
    ...
    pending = str(
        int(orders.filter(status='Pending').count())
        + int(orders.filter(status='Out for delivery').count())
        )
    ...
    return render(request, 'accounts/dashboard.html', context)  # filepath to render dashboard.html template
...
def alexandra(request):
    return render(request, 'accounts/alexandra.html')
